...
#ifdef PYUNIKEYNT_EXPORTS
#define vu_api extern "C" __declspec(dllexport)
#else  // PYUNIKEYNT_EXPORTS
#define vu_api extern "C" __declspec(dllimport)
#endif // PYUNIKEYNT_EXPORTS
...
vu_api vu::VUResult __cdecl unikey_nt_initialize(vu::ulong process_id)
{
  ...
  if (process_id == 0)
  {
    auto pids = vu::name_to_pid_A("UniKeyNT.exe");
    if (!pids.empty())
    {
      process_id = pids.front();
    }
  }
...
vu_api int unikey_nt_get_current_mode()
{
  if (!g_unikey_nt.initialized)
  {
...
vu_api void __cdecl unikey_nt_set_current_mode(int mode)
{
  if (!g_unikey_nt.initialized)
  {
    vu::print_A("Unikey NT:error that did not initialize");
  ...
  g_unikey_nt.process.execute_code_at(g_unikey_nt.address_change_mode_function, nullptr, true);
}
